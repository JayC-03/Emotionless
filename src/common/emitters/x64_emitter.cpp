...
void OpArg::WriteREX(XEmitter* emit, int opBits, int bits, int customOp) const
{
	    ...
	    (opBits == 8 && (customOp & 0x10c) == 4))
	{
		...
		//assert((offsetOrBaseReg & 0x100) == 0);
		//assert((customOp & 0x100) == 0);
	}
}
	...
	bool warn_64bit_offset) const
{
	...
	if (SIB)
	{
		...
		default: break; //assert(0, "Invalid scale for SIB byte"); ss = 0; break;
		}
...
void XEmitter::JMP(const u8* addr, bool force5Bytes)
{
	...
	else
	{
		...
		//assert(distance >= -0x80000000LL && distance < 0x80000000LL,
...
void XEmitter::CALL(const void* fnptr)
{
	...
	//assert(distance < 0x0000000080000000ULL ||
...
{
	if (arg.IsImm()) //assert(DYNA_REC, 0, "INC - Imm argument");
	arg.operandReg = 0;
...
{
	if (arg.IsImm()) //assert(DYNA_REC, 0, "DEC - Imm argument");
	arg.operandReg = 1;
	if (bits == 16) {Write8(0x66);}
	arg.WriteREX(this, bits, bits);
	Write8(bits == 8 ? 0xFE : 0xFF);
...
void XEmitter::WriteMOVBE(int bits, u8 op, X64Reg reg, const OpArg& arg)
{
	...
	if (bits == 16)
		Write8(0x66);
	//assert(!arg.IsSimpleReg() && !arg.IsImm(), "MOVBE: need r<-m or m<-r!");
...
void OpArg::WriteNormalOp(XEmitter* emit, bool toRM, NormalOp op, const OpArg& operand, int bits) const
{
	...
	if (operand.IsImm())
	{
		...
		{
			//assert(0, "WriteNormalOp - Unhandled case %d %d", operand.scale, bits);
		}
...
void XEmitter::MOV (int bits, const OpArg& a1, const OpArg& a2)
{
	if (a1.IsSimpleReg() && a2.IsSimpleReg() && a1.GetSimpleReg() == a2.GetSimpleReg())
		log_print("x64 Emitter", log_level::error, "Redundant MOV @ %p - bug in JIT?", code);
...
void XEmitter::WriteFloatLoadStore(int bits, FloatOp op, FloatOp op_80b, const OpArg& arg)
{
	...
	case 64: mf = 4; break;
	case 80: mf = 2; break;
	default: break; //assert(0, "WriteFloatLoadStore: invalid bits (should be 32/64/80)");
	}
	Write8(0xd9 | mf);
